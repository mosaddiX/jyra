...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Need help with Jyra? First, please provide a subject for your ticket:"
    )

    # Set the next handler
    context.user_data["expecting_support_subject"] = True
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif context.user_data.get("expecting_support_subject"):
        # Handle support subject
        # Store the subject
        context.user_data["support_subject"] = message_text

        # Ask for description
        await update.message.reply_text(
            "Thank you for the subject. Now, please provide a detailed description of your issue:"
        )

        # Update the flag
        context.user_data["expecting_support_subject"] = False
        context.user_data["expecting_support_description"] = True
        return True

    elif context.user_data.get("expecting_support_description"):
        # Handle support description
...
def register_community_handlers(application):
    ...
    application.add_handler(CommandHandler(
        "featurerequest", feature_request_command))
    application.add_handler(CommandHandler(
        "generalfeedback", general_feedback_command))
