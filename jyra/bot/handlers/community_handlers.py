...
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💡 Feature Suggestion", callback_data="feedback_feature")],
...
async def ticket_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("📝 Add Response", callback_data=f"respond_{ticket_id}")],
        [InlineKeyboardButton("✅ Mark as Resolved", callback_data=f"resolve_{ticket_id}")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def register_community_handlers(application):
    ...
    feedback_conv_handler = ConversationHandler(
        ...
        states={
            FEEDBACK_TYPE: [
                CallbackQueryHandler(feedback_type_callback, pattern=r"^feedback_"),
            ...
            FEEDBACK_RATING: [
                CallbackQueryHandler(feedback_rating_callback, pattern=r"^rating_"),
                CallbackQueryHandler(cancel_callback, pattern=r"^cancel_")
            ]
        },
    ...
    feature_conv_handler = ConversationHandler(
        ...
        states={
            ...
            FEATURE_DESCRIPTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, feature_description)]
        },
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
    ...
    support_conv_handler = ConversationHandler(
        ...
        states={
            ...
            SUPPORT_PRIORITY: [
                CallbackQueryHandler(support_priority_callback, pattern=r"^priority_"),
                CallbackQueryHandler(cancel_callback, pattern=r"^cancel_")
            ]
        },
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
